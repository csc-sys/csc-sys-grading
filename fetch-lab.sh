...
for usr in $dirs; do
  ...
  case $lab; in
    ...
    stacklab)
      try tar --ignore-failed-read -cf $destdir/$usr-$lab.tar -C $home/ --exclude=Makefile stacklab/ --transform='s@^stacklab/@@' .history .bash_history .zhistory .zsh_history .gdb_history .stacklab;;
    locksmith)
      try tar cf $destdir/$usr-$lab.tar -C $home/submissions $usr-locksmith.txt;;
    *)
      try cp -af $home/submissions/(#i)$usr-$lab.tar $destdir/$usr-$lab.tar;;
  esac
done
